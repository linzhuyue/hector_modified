...
void ConvexMPCLocomotion::run(ControlFSMData &data)
{
  ...
  for (int foot = 0; foot < 2; foot++)
  {
    ...
    if (swingState > 0) // foot is in swing
    {
      ...
      // in base frame try to modify this
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int *mpcTable, ControlFSMData &data, bool omniMode)
{
  ...
  if ((iterationCounter % 5) == 0)
  {
    ...
    Eigen::Matrix<double, 18, 1> footrotation;
    footrotation.setZero();
    ...
    Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(footrotation.data()) = placementL.rotation();
    Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(footrotation.data() + 9) = placementR.rotation();
    double *footrotations = footrotation.data();
    ...
    Timer t_mpc_solve;
    t_mpc_solve.start();
    update_problem_data(p, v, quat, w, r, footrotations ,yaw, weights, trajAll, Alpha_K, mpcTable);
